...
### 1단계: 입력 파일 읽기 (v3 → v2 폴백) + 출처 메타 확인
...
txt_v2 레거시 파일은 헤더 없음 — 본문 전체를 파싱 대상으로 사용하고 SQL 주석에 `-- 출처: bokziri_legacy` 로 기재.

`safe_name` 규칙 (`server/tools/discover_and_scrape.py:116` 구현과 동일):
...
3. 연속된 `_` → 단일 `_`
4. 양끝 `_` 제거
5. 결과가 빈 문자열이면 `unknown`

**보존되는 문자** (변환 안 됨): `-`, `(`, `)`, `.`, `,`, `㈜`, 한글, 영문, 숫자
...
- `CJ ENM` → `CJ_ENM`
- `S-Oil` → `S-Oil` (하이픈 보존)
- `CJ제일제당(주)` → `CJ제일제당(주)` (괄호 보존)
- `LG  전자` (공백 2개) → `LG_전자`

**중요**: 이 규칙은 **txt_v3 파일 경로 조회 전용**입니다. DB INSERT의 `COMP_NM` 컬럼과 SQL 주석에는 **원본 회사명**을 그대로 사용합니다. COMP_ENG_NM은 --eng 인자 또는 DB 조회로 별도 결정.

두 위치 모두 없거나 모두 <200 bytes 인 경우 사용자에게 안내:
```
...
다음 파일들을 **병렬**로 읽습니다:
- `server/seed/benefit/sql/삼성전자.sql` — **항목 선정 · 분량 · 주석 스타일 참고용**. 단, 이 파일은 레거시 포맷(10-컬럼, 세션변수 없음)이며 **SQL 출력 포맷은 반드시 아래 5단계 템플릿(14-컬럼, 세션변수 포함)을 따른다**. 포맷이 충돌할 때 5단계가 우선.
