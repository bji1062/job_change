...
// ━━ SEARCH ━━
function doSearch(s){const q=document.getElementById(s==='a'?'sA':'sB').value.trim().toLowerCase();const el=document.getElementById(s==='a'?'rA':'rB');if(q.length<1){el.classList.remove('open');return}const m=DB.filter(c=>c.COMP_NM.toLowerCase().includes(q)||c.ALIAS_NM_LIST.some(a=>a.toLowerCase().includes(q)));if(m.length){el.innerHTML=m.map(c=>`<div class="si" onclick="selComp('${s}','${c.COMP_ENG_NM}')"><div class="si-logo">${c.LOGO_NM}</div><div class="si-info"><div class="si-name">${c.COMP_NM}</div><div class="si-meta">${c.INDUSTRY_NM}</div></div><span class="si-badge">DB</span></div>`).join('')}else{el.innerHTML=`<div class="sr-empty">검색 결과 없음<span style="display:block;font-size:.72rem;color:var(--t4);margin-top:4px">기업 유형을 선택하면 평균 복지를 채워드려요</span></div>`}el.classList.add('open')}
function selComp(s,id){const c=DB.find(x=>x.COMP_ENG_NM===id);if(!c)return;matched[s]=c;document.getElementById(s==='a'?'sA':'sB').value=c.COMP_NM;document.getElementById(s==='a'?'rA':'rB').classList.remove('open');document.getElementById(s==='a'?'tA':'tB').value=c.COMP_TP_CD;document.getElementById(s==='a'?'mA':'mB').classList.add('show');document.getElementById(s==='a'?'pA':'pB').classList.remove('show');benS[s]=c.BENEFIT_LIST.map(b=>({...b,checked:b.BENEFIT_AMT>0}));renderBen(s);
  const ws=c.WORK_STYLE_JSON||{};const type=c.COMP_TP_CD;
  const preset=WORK_PRESETS[type]||{};
  ...
  const pr=document.getElementById('wsPr'+s.toUpperCase());
  pr.innerHTML=`✓ <strong>${c.COMP_NM}</strong> DB 근무방식 + ${TYPE_LABELS[type]} 평균 추정 적용`;pr.classList.add('show');
  calc()}
...
// ━━ BENEFITS ━━
function renderBen(s){const list=benS[s];if(!list?.length){document.getElementById(s==='a'?'blA':'blB').innerHTML='';document.getElementById(s==='a'?'bcA':'bcB').textContent='';return}const grouped={};list.forEach((b,i)=>{if(!grouped[b.CATEGORY_CD])grouped[b.CATEGORY_CD]=[];grouped[b.CATEGORY_CD].push({...b,idx:i})});let h='';for(const[cat,items]of Object.entries(grouped)){h+=`<div class="bg"><div class="bg-label">${CAT_LABELS[cat]||cat}</div>`;items.forEach(b=>{h+=`<div class="bi"><input type="checkbox" class="bi-ck" ${b.checked?'checked':''} onchange="togBen('${s}',${b.idx},this.checked)"><span class="bi-n">${b.BENEFIT_NM}${b.NOTE_NM?`<span class="bi-note">${b.NOTE_NM}</span>`:''}</span><span class="bi-badge ${b.BADGE_CD==='auto'?'b-auto':'b-est'}">${b.BADGE_CD==='auto'?'DB':'추정'}</span><input type="number" class="bi-v" value="${b.BENEFIT_AMT}" onchange="setBenV('${s}',${b.idx},this.value)"><span class="bi-u">만</span></div>`});h+='</div>'}const isPreset=!matched[s]&&list.length>0;if(isPreset){const type=document.getElementById(s==='a'?'tA':'tB').value;h+=`<div class="ben-note-preset">모든 금액은 <strong>${TYPE_LABELS[type]||type} 평균 추정치</strong>입니다. 실제 금액을 아는 항목만 수정하세요. 체크 해제 항목은 합산 제외.</div>`}document.getElementById(s==='a'?'blA':'blB').innerHTML=h;const ac=list.filter(b=>b.checked&&b.BENEFIT_AMT>0).length;document.getElementById(s==='a'?'bcA':'bcB').textContent=isPreset?`(${ac}/${list.length}개 · 평균 추정)`:`(${ac}개 항목)`}
function togBen(s,i,c){benS[s][i].checked=c;calc()}
function setBenV(s,i,v){benS[s][i].BENEFIT_AMT=Number(v)||0;calc()}
function getBenTotal(s){let t=0;(benS[s]||[]).forEach(b=>{if(b.checked)t+=b.BENEFIT_AMT});return{ben:t,net:t}}
